from src.adapters.inner_bus import AsyncioConsumer, RAW_COMMAND_HANDLERS, RAW_EVENT_HANDLERS
...
def bootstrap(s3: S3ABC, producer: BaseProducer) -> AsyncioConsumer:
    ...
    for command, handler in RAW_COMMAND_HANDLERS.items():
        injected_object = inject(handler_class=handler, injects=injects)
        injected_command_handlers[command] = injected_object
    
    for event, handlers in RAW_EVENT_HANDLERS.items():
        injected_event_handlers[event] = [inject(handler_class=handler, injects=injects) for handler in handlers]
...
def inject(handler_class: object, injects: Dict[str, object]) -> object:
    ...
    values_for_object_creating = {key: injects[key] for key in injects if key in params}
