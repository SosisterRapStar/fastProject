from src.adapters.inner_bus import (
    AsyncioConsumer,
...
def bootstrap(s3: S3ABC, producer: BaseProducer) -> AsyncioConsumer:
    injects = {"s3": s3(), "producer": producer()}
    ...
    for event, handlers in RAW_EVENT_HANDLERS.items():
        injected_event_handlers[event] = [
            inject(handler_class=handler, injects=injects) for handler in handlers
        ]

    message_bus = AsyncioConsumer(
