...
import contextvars
from crud.conversation_repository import AbstractConversationRepository
...
class ChatService:
    ...
    def __init__(
        self, conv_repo: AbstractConversationRepository, manager: "ConnectionManager"
    ):
        self.conv_repo = conv_repo
        self.manager = manager

    async def handle_message_from_user(
        self, message: "MessageFromBroker"
    ):  # message может быть и json
        users_list = await self.conv_repo.get_users(
            conv_id=message.conversation_id, selectable="id"
        )
